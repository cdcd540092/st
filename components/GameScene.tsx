...
const GameScene: React.FC<GameSceneProps> = ({
      ...
      if (currentZ > PLAYER_Z - 1.5 && currentZ < PLAYER_Z + 1.0) {
        ...
        if (handPos) {
          ...
          if (handPos.distanceTo(notePos) < 0.8) {
            ...
            // Speed check: Maintain a very low threshold to ensure hand is active,
            // but don't skip if the movement is slow, allowing "touch" hits.
            if (speed < 0.1) continue;

            // Grip check
            const isGripping = note.type === 'left' ? hands.leftGripping : hands.rightGripping;

            if (note.requiresGrip) {
              if (!isGripping) {
                // Requires grip but not gripping -> Skip (Miss later or hit later)
                continue;
              }
            } else {
              if (isGripping) {
                // Does NOT require grip but user IS gripping -> Bad hit (Penalty)
                goodCut = false;
              }
            }

            // Directional check (if applicable)
  ...
  return (
    <>
      ...
      <Stars radius={100} depth={20} count={5000} factor={4} saturation={1} fade speed={2} />
      <group rotation={[0, 0, Math.PI / 4]}>
        <Stars radius={150} depth={50} count={2000} factor={2} saturation={0} fade speed={0.5} />
      </group>
